...
use crate::grammar::twig::literal::{
    parse_postfix_operators, parse_twig_arrow_function, parse_twig_literal, parse_twig_name,
};
...
fn parse_paren_expression(parser: &mut Parser) -> (Marker, bool) {
    ...
    // first check for a unary expression token
    if parser
        .peek_token()
        .map_or(true, |t| t.kind.unary_binding_power().is_none())
    {
        // check for a simple name literal
        if let Some(lit_m) = parse_twig_name(parser) {
            if parser.at(T![","]) || parser.at_following(&[T![")"], T!["=>"]]) {
                return (m, false);
            }

            // including postfix operators
            let lit_m = parse_postfix_operators(parser, lit_m);
